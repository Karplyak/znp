  ...
  */
#endif
  {
    ...
    case 3:
      if (serialNumber[0] == 0)
      {
        uint8 extAddr[Z_EXTADDR_LEN], idx;
        uint16 tmp;

        HalFlashRead(HAL_FLASH_IEEE_PAGE, HAL_FLASH_IEEE_OSET, extAddr, Z_EXTADDR_LEN);
        for (idx = 2; idx < 42; idx += 2)
        {
          if (((idx-2) % 10) == 0)
          {
            uint8 idx2 = (idx-2) / 5;
            tmp = BUILD_UINT16(extAddr[idx2], extAddr[idx2+1]);
          }
          
          serialNumber[idx] = '0' + (tmp % 10);
          tmp /= 10;
        }
        serialNumber[0] = 42;
      }
